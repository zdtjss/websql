...
var SQL_DIALECT = map[string]map[string]string{
	...
	"oracle": {
		...
		"listIndexes":     "SELECT i.INDEX_NAME,ic.COLUMN_NAME,case when i.UNIQUENESS='UNIQUE' then 0 else 1 end NON_UNIQUE,ic.COLUMN_POSITION SEQ_IN_INDEX,i.INDEX_TYPE,'' NULLABLE,'' COMMENT,'' INDEX_COMMENT FROM USER_INDEXES i JOIN USER_IND_COLUMNS ic ON i.INDEX_NAME=ic.INDEX_NAME WHERE i.TABLE_NAME = :1 ORDER BY i.INDEX_NAME,ic.COLUMN_POSITION",
		"schemaOverview":  "SELECT tc.TABLE_NAME,utc.TABLE_TYPE,tc2.COMMENTS TABLE_COMMENT,b.COLUMN_NAME,b.DATA_TYPE COLUMN_TYPE,CASE WHEN uc.CONSTRAINT_TYPE='P' THEN 'PRI' ELSE '' END COLUMN_KEY,a.COMMENTS COLUMN_COMMENT,CASE WHEN b.NULLABLE='Y' THEN 'YES' ELSE 'NO' END IS_NULLABLE FROM USER_TAB_COMMENTS tc2 JOIN (SELECT TABLE_NAME,'TABLE' TABLE_TYPE FROM USER_TABLES UNION ALL SELECT VIEW_NAME,'VIEW' FROM USER_VIEWS) utc ON tc2.TABLE_NAME=utc.TABLE_NAME JOIN USER_TAB_COLUMNS b ON tc2.TABLE_NAME=b.TABLE_NAME JOIN USER_COL_COMMENTS a ON a.TABLE_NAME=b.TABLE_NAME AND a.COLUMN_NAME=b.COLUMN_NAME LEFT JOIN (SELECT ucc.TABLE_NAME,ucc.COLUMN_NAME,uc2.CONSTRAINT_TYPE FROM USER_CONS_COLUMNS ucc JOIN USER_CONSTRAINTS uc2 ON ucc.CONSTRAINT_NAME=uc2.CONSTRAINT_NAME WHERE uc2.CONSTRAINT_TYPE='P') uc ON b.TABLE_NAME=uc.TABLE_NAME AND b.COLUMN_NAME=uc.COLUMN_NAME JOIN USER_TAB_COLUMNS tc ON tc.TABLE_NAME=b.TABLE_NAME AND tc.COLUMN_NAME=b.COLUMN_NAME WHERE 'notexists' <> :1 ORDER BY tc2.TABLE_NAME,tc.COLUMN_ID",
	},
}
