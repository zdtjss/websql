...
//   - Agent 通过 skill 工具（Eino Skill Middleware）加载 SKILL.md
//   - Agent 通过 execute 工具（Eino Filesystem Middleware）执行 Python 脚本
//   - Go 端 NewExportAnalysisDocxFunc / NewExportPPTFunc 仅作为 Python 不可用时的
//     原生兜底实现（见 tools.go）
//
// 因此本文件的编排函数已全部移除，仅保留被 tools.go 复用的 cleanupFiles。
// SkillEnv / IsPythonAvailable 等基础能力保留在 skill_detector.go。
