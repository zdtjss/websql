...
func RunMigrations(db *sqlx.DB, driverName string, fsys fs.FS) error {
	m := &Migrator{db: db, driverName: driverName, fsys: fsys}
	return m.run()
}
...
func (m *Migrator) run() error {
	...
	// 存量库首次接入：无迁移记录但已有业务表 → 标记基线已执行，跳过 baseline。
	if len(applied) == 0 {
