...
func buildDictTable(conn *sqlx.DB, dbType, schema, table string) DictTable {
	...
	colSQL, ok := dialect.SQL_DIALECT[dbType]["listTableColumns"]
	if !ok {
	...
	case "oracle":
		...
		if err != nil {
			logger.PrintErrf("获取列信息失败: %v", err)
			return dt
		}
		defer rows.Close()
		i := 0
	...
	idxSQL, ok := dialect.SQL_DIALECT[dbType]["listIndexes"]
	if ok {
		type IdxRow struct {
