...
func main() {
	...
	mngtDB := database.Mngtdb // 本地持有引用，后续传入 Container（过渡期仍需设置全局变量供 LoadConfigFromDB 等使用）
	...
	if err := migration.RunMigrations(mngtDB, config.Get().DB.DriverName, migrationSubFS, string(fullInitSQL)); err != nil {
