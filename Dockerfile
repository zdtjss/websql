...
RUN sed -i "s@http://.*archive.ubuntu.com@http://mirrors.ustc.edu.cn@g" /etc/apt/sources.list && sed -i "s@http://.*security.ubuntu.com@http://mirrors.ustc.edu.cn@g" /etc/apt/sources.list && \
    ...
    cd web-src && npm install --registry=https://registry.npmmirror.com && npm run build && mv dist /app/static && \
    rm -rf /app/websql && rm -f /app/go1.20.1.linux-amd64.tar.gz && rm -rf /usr/local/go && rm -f /app/node-v18.14.1-linux-x64.tar.gz && rm -rf /usr/local/node-v18.14.1-linux-x64 && apt-get autoremove -y gcc 
