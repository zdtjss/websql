-- 基线迁移：管理库初始 schema + 种子数据
-- 等价于原 sqlite3-init.sql，新增 t_schema_migration 版本记录表。
-- 仅在全新库（无任何业务表）时执行；存量库首次接入迁移系统时自动标记为已执行，跳过本脚本。
...
insert or ignore into t_role (id, name, view_classic, allow_modify) values ('825683877266722816', 'admin', 1, 1);
...
INSERT OR IGNORE INTO "t_user" ("id", "login_name", "name", "pwd","bio") VALUES ('825683877312860160', 'admin', '管理员', '9c122652077a13080ab640b549cc64d0','');

-- local 用户，用于本地/桌面模式自动登录，密码是 1，属于 admin 角色
INSERT OR IGNORE INTO "t_user" ("id", "login_name", "name", "pwd","bio") VALUES ('825683877312860161', 'local', 'local', '7e2e1f2e1eb71a6f7915a96201237ff0','');
...
INSERT OR IGNORE INTO "t_user_role" ("id", "user_id", "role_id") VALUES ('825683877367386112', '825683877312860160', '825683877266722816');

-- local 用户绑定 admin 角色
INSERT OR IGNORE INTO "t_user_role" ("id", "user_id", "role_id") VALUES ('825683877367386113', '825683877312860161', '825683877266722816');
