<template>
  <!-- 服务器变量 Tab：全局 / 会话变量，可搜索 -->
  <div v-loading="loading" :aria-busy="loading" style="min-height: 200px;" role="region" aria-label="服务器变量">
    ...
    <template v-else>
      <div class="vars-toolbar">
        ...
        <el-input v-model="filter" placeholder="按变量名或值过滤" size="small" clearable style="width: 260px;" aria-label="过滤变量列表" />
        <el-button size="small" @click="load" :loading="loading" aria-label="刷新变量列表">刷新</el-button>
      ...
      <el-table :data="filteredList" max-height="440" size="small" stripe border aria-label="服务器变量列表">
      ...
      <el-empty v-if="!loading && filteredList.length === 0" description="没有符合条件的变量" :image-size="60" />
      <!-- AI 分析结果区域 -->
...
import { getMonitorAllVariables } from '@/api/conn'
...
const list = ref<VarItem[]>([])
const filter = ref('')
const scope = ref<'global' | 'session'>('global')
...
async function load() {
  ...
  loading.value = true
  try {
    const res = await getMonitorAllVariables(props.connId, scope.value)
    const data = res.data?.data || {}
    if (data.dbType) dbType.value = data.dbType
    if (data.version) dbVersion.value = data.version
    if (data.supported === false) {
      list.value = []
      unsupported.value = data.unsupportedMessage || '当前数据库不支持查看服务器变量'
    } else {
