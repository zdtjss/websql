...
func SearchObjects(c *gin.Context) {
	...
	if strings.TrimSpace(keyword) == "" {
		utils.WriteJson(c.Writer, map[string]any{"results": []ObjectSearchResult{}, "totalResults": 0})
		return
	}
	...
	if searchType == "all" || searchType == "table" {
		sqlTmpl, _ := dbutils.SQL_DIALECT[dbType]["listTable"]
		switch dbType {
		case "oracle":
...
func SearchData(c *gin.Context) {
	...
	if strings.TrimSpace(keyword) == "" {
		utils.WriteJson(c.Writer, map[string]any{"results": []DataSearchResult{}, "totalResults": 0, "searchedTables": 0})
		return
	}
...
func SearchAll(c *gin.Context) {
	...
	if strings.TrimSpace(keyword) == "" {
		utils.WriteJson(c.Writer, &SearchResponse{
			Query:         keyword,
			SearchType:    searchType,
			TotalResults:  0,
...
func GetSearchTables(c *gin.Context) {
	...
	utils.WriteJson(c.Writer, map[string]any{"tables": infos})
}
