...
func ensureBackupTable() {
	migrateOnce.Do(func() {
		...
		var hasNameCol bool
		row := config.Mngtdb.QueryRow("SELECT COUNT(*) > 0 FROM pragma_table_info('t_backup') WHERE name='name'")
		if err := row.Scan(&hasNameCol); err != nil {
			var colCount int
			row2 := config.Mngtdb.QueryRow("SELECT COUNT(*) FROM information_schema.columns WHERE table_name='t_backup' AND column_name='name'")
		...
		config.Mngtdb.Exec("DROP TABLE IF EXISTS t_backup")
		config.Mngtdb.Exec(`CREATE TABLE t_backup (
			id TEXT PRIMARY KEY,
			name TEXT,
			conn_id TEXT,
			schema_name TEXT,
...
func CreateBackup(c *gin.Context) {
	...
	for _, table := range tables {
		...
		if withData == "true" {
			...
			if err != nil {
				errors = append(errors, fmt.Sprintf("导出表 %s 数据失败: %s", table, err.Error()))
				continue
			}
			if rowCount > 0 {
	...
	if encrypt == "true" {
		encryptedContent := utils.AESEncode(string(content))
	...
	if err4 != nil {
		logutils.PrintErrf("保存备份记录失败", err4)
		config.Mngtdb.Exec(
...
func ListBackups(c *gin.Context) {
	...
	var err error
	if connId != "" && schema != "" {
		err = config.Mngtdb.Select(&records, "SELECT id,name,conn_id,schema_name,db_type,size_bytes,backup_type,encrypted,created_at,description,COALESCE(status,'completed') status, COALESCE(file_path,'') file_path FROM t_backup WHERE conn_id=? AND schema_name=? ORDER BY created_at DESC", connId, schema)
	} else if connId != "" {
		err = config.Mngtdb.Select(&records, "SELECT id,name,conn_id,schema_name,db_type,size_bytes,backup_type,encrypted,created_at,description,COALESCE(status,'completed') status, COALESCE(file_path,'') file_path FROM t_backup WHERE conn_id=? ORDER BY created_at DESC", connId)
	} else {
		err = config.Mngtdb.Select(&records, "SELECT id,name,conn_id,schema_name,db_type,size_bytes,backup_type,encrypted,created_at,description,COALESCE(status,'completed') status, COALESCE(file_path,'') file_path FROM t_backup ORDER BY created_at DESC")
	}
	if err != nil {
...
func RestoreBackup(c *gin.Context) {
	...
	err := config.Mngtdb.Get(&record, "SELECT * FROM t_backup WHERE id=?", backupId)
	if err != nil {
	...
	if err != nil && record.Encrypted {
		utils.WriteJson(c.Writer, map[string]any{"success": false, "message": "备份文件不存在"})
		return
	}

	sqlContent := string(content)
	if record.Encrypted {
		sqlContent = utils.AESDecode(sqlContent)
	}
...
func DeleteBackup(c *gin.Context) {
	...
	_, err = config.Mngtdb.Exec("DELETE FROM t_backup WHERE id=?", backupId)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "msg": "删除备份记录失败: " + err.Error()})
		return
	}

	utils.WriteJson(c.Writer, map[string]any{"success": true})
}
...
func DownloadBackup(c *gin.Context) {
	...
	if record.Encrypted {
		content = []byte(utils.AESDecode(string(content)))
	}
