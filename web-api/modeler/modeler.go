...
func ReverseEngineer(c *gin.Context) {
	...
	utils.WriteJson(c.Writer, model)
}
...
func buildERTable(conn *sqlx.DB, dbType, schema, tableName string) ERTable {
	...
	case "oracle":
		...
		logutils.PanicErrf("获取列信息失败", err)
		for _, c := range cols {
			isPK := false
...
func extractRelations(conn *sqlx.DB, dbType, schema string) []ERRelation {
	...
	if err != nil {
		logutils.PrintErrf("获取外键关系失败", err)
		return relations
	}
	defer rows.Close()
...
func ForwardEngineer(c *gin.Context) {
	...
	if strings.TrimSpace(ddlSql) == "" {
		utils.WriteJson(c.Writer, map[string]any{"success": false, "message": "DDL不能为空"})
		return
	}
...
func ExportModel(c *gin.Context) {
	...
	case "ddl":
		...
		utils.WriteJson(c.Writer, map[string]any{"ddl": ddl, "format": "ddl"})
	case "json":
		utils.WriteJson(c.Writer, map[string]any{"model": model, "format": "json"})
	default:
		utils.WriteJson(c.Writer, map[string]any{"model": model, "format": format})
	}
}
