...
func CompareData(c *gin.Context) {
	...
	conn1 := admin.GetConn(connId1, authorization)
	conn2 := admin.GetConn(connId2, authorization)
	...
	if sourceCount > maxCompareRows || targetCount > maxCompareRows {
		utils.WriteJson(c.Writer, map[string]any{
			"error": fmt.Sprintf("数据量过大，源表%d行/目标表%d行，上限%d行，请使用数据导出功能", sourceCount, targetCount, maxCompareRows),
		})
		return
	}
	...
	utils.WriteJson(c.Writer, pagedResult)
}
...
func queryAllData(conn *sqlx.DB, dbType string, sql string) []map[string]any {
	...
	if err != nil {
		logutils.PrintErrf("数据查询失败", err)
		return nil
	}
	defer rows.Close()
	return dbutils.GetResultRows(dbType, rows)
}
...
func ApplyDataSync(c *gin.Context) {
	...
	if strings.TrimSpace(sqlStr) == "" {
		utils.WriteJson(c.Writer, map[string]any{"success": false, "message": "SQL不能为空"})
		return
	}
	...
	for _, s := range sqlList {
		...
		if err := validateDataSQL(s); err != nil {
			utils.WriteJson(c.Writer, map[string]any{"success": false, "message": err.Error()})
			return
		}
	...
	tx, err := conn.Beginx()
	if err != nil {
		utils.WriteJson(c.Writer, map[string]any{"success": false, "message": fmt.Sprintf("开启事务失败: %v", err)})
		return
	}
...
func GenerateSyncSQL(c *gin.Context) {
	...
	if sourceCount > maxCompareRows || targetCount > maxCompareRows {
		utils.WriteJson(c.Writer, map[string]any{
			"error": fmt.Sprintf("数据量过大，源表%d行/目标表%d行，上限%d行", sourceCount, targetCount, maxCompareRows),
		})
		return
	}
...
func CompareDataChunked(c *gin.Context) {
	...
	if table == "" {
		utils.WriteJson(c.Writer, map[string]any{"error": "表名不能为空"})
		return
	}
	if !isValidIdentifier(table) {
		utils.WriteJson(c.Writer, map[string]any{"error": "表名包含非法字符"})
		return
	}

	keyColumns := getKeyColumns(conn1, dbType, schema1, table, keyColumnsStr)
	if len(keyColumns) == 0 {
		utils.WriteJson(c.Writer, map[string]any{"error": "无法确定比较键列，请确保表有主键或指定keyColumns参数"})
		return
	}
