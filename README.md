...
flowchart TD
    ...
    subgraph B["SQLAgent - ChatModelAgent / ReAct Loop"]
        direction TB
        B1["System Prompt<br/>数据库类型 / Schema / 权限描述 / 安全规则"]
        B2["8 个内置工具"]
        B2a["query_data - SELECT / SHOW 等"]
        B2b["exec_sql - INSERT / UPDATE / DELETE"]
        B2c["get_table_schema - 建表语句 & 结构"]
        B2d["export_excel - 纯数据 Excel"]
        B2e["export_excel_with_chart - Excel + 图表"]
        B2f["export_ppt - PPTX 演示文稿"]
        B2g["export_analysis_docx - Word 数据报告"]
        B2h["import_data - Excel 导入"]
        B3["4 层中间件链"]
        B3a["PermissionMiddleware - 调用权限审核 Agent"]
        B3b["DangerousSQLApproval - 写操作中断确认"]
        B3c["ToolErrorRecovery - 工具错误自动重试"]
        B3d["Summarization - 超 10 万 Token 摘要"]
    end

    B --> C["每次工具调用"]
    C --> D

    subgraph D["PermissionAgent - 权限审核智能体"]
        direction TB
        D1["System Prompt<br/>四级权限规则 / 最具体优先 / 判断速查"]
        D2["2 个工具"]
        D2a["get_table_structure - 查询目标表的列结构"]
        D2b["get_user_permissions - 获取用户数据权限配置"]
        D3["输出: allowed, deniedTables, deniedColumns"]
        D4["失败时自动降级 → 程序化权限检查<br/>正则 + 查表"]
    end

    D --> E["SSE 流式输出<br/>session / thinking / content / danger_confirm / done"]
```
