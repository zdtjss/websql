...
- **国产化迫切**：国外工具授权成本高、存在断供风险，信创替代需求迫切
...
WebSQL 定位于 **"AI 原生、自主可控、单文件即服务"** 的企业级数据库智能管理平台，适用于数据查询分析、报表自动化生成、数据库日常运维、数据安全保障四大典型场景。
...
| **自主可控** | 单文件部署于自有服务器，数据不出服务器，支持本地离线 AI 运行                |
| **信创友好** | MIT 开源协议，Go + Vue 标准技术栈，纯 Go 编译无外部运行时依赖，可适配信创环境 |
| **国产替代** | 对标国外商业数据库工具，无授权绑定、无遥测代码、无供应商锁定                  |
...
WebSQL 秉承 **"AI 原生、单文件即服务、数据不出域"** 三大设计理念，以工程化方式将大模型能力深度嵌入数据库管理全链路，而非简单的外挂式调用。
...
| **数据库引擎**   | 纯 Go SQLite（modernc.org/sqlite）       | 无 CGO 依赖，跨平台编译，印证"零外部依赖"                  |
...
- **单文件即服务**：Go 编译产物内嵌前端静态资源，一个二进制即可启动完整服务，无需 Node、无需 Python、无需数据库驱动安装
- **零运行时依赖**：纯 Go 编译，无外部运行时依赖，可快速部署于物理服务器、虚拟化平台及容器化环境
- **数据主权可控**：所有数据不离开服务器，支持本地 Ollama 离线运行 AI，满足数据不出域的合规要求
...
- **数据主权可控**：单文件部署于自有服务器，所有数据不离开服务器；支持本地 Ollama 离线运行 AI，满足数据不出域要求
- **零遥测代码**：不包含任何遥测、行为追踪或数据收集代码，代码完全公开可审计
- **零供应商锁定**：基于标准 SQL 与 HTTP/SSE 协议，Go + Vue 3 标准技术栈，无商业授权服务器依赖
- **信创环境适配**：纯 Go 编译，无 CGO 依赖，可适配信创操作系统与硬件环境
